...
这两种方法 `spread` 语法显然更优雅，但是在 [jsperf 性能基准测试](https://jsperf.com/30-seconds-for-everyday-comparing-object-asign-spread) 中 `Object.assign()` 肉眼可见的快了50%-60%。
...
很显然 `Array.reduce()` 的写法更优雅，但是在 [jsperf 性能基准测试](https://jsperf.com/30-seconds-for-everyday-for-loop-vs-reduce) 中 `for 循环` 肉眼可见的快了90%。
...
* 编写实例代码或者教程、类库、个人项目时，考虑到读者的水平不一定都一样时，也可以优先`可读性`在后续文章或者迭代中补充说明。

#### 什么时候性能优先

* 其实性能优先和可读性优先都是`相互`的，在处理`大量数据`时就需要考虑性能了。有的同学会说项目哪里有这么多数据，还真别说小二做过一个非常蛋疼的项目，后端直接返回几万条数据让前端来处理。别说话吻我，我不想回忆这段过去了。
* 当项目在运行时发现了特别明显的卡顿等性能问题时。
* 更多的取舍应该要出于项目实际情况进行选择。
